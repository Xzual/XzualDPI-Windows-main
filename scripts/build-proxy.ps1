# Xzual proxy (SpoofDPI 1.2.1) - PowerShell ile derleme ve kopyalama
# KullanÄ±m: PowerShell'de proje kÃ¶kÃ¼nde .\scripts\build-proxy.ps1
# veya script'e saÄŸ tÄ±k -> "PowerShell ile Ã‡alÄ±ÅŸtÄ±r"

$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$Root = Split-Path -Parent $ScriptDir
$SpoofDpiSrc = Join-Path $Root "SpoofDPI-1.2.1\SpoofDPI-1.2.1"
$OutExe = Join-Path $Root "spoofdpi\xzual-proxy.exe"
...
Write-Host "SpoofDPI (xzual-proxy) derleniyor..." -ForegroundColor Cyan
...
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "xzual-proxy.exe") -Force
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "xzual-proxy-x86_64-pc-windows-msvc.exe") -Force
Write-Host "Kopyalandi: src-tauri\binaries\xzual-proxy.exe (ve -x86_64-pc-windows-msvc.exe)" -ForegroundColor Green
