...
const BandwidthVisualizer = ({ isConnected, isProcessing }) => {
  const [data, setData] = useState(Array(20).fill(0));
  const [speed, setSpeed] = useState(0);

  useEffect(() => {
    if (!isConnected) {
      setData(Array(20).fill(0));
      setSpeed(0);
      return;
    }
    ...
    const interval = setInterval(() => {
      setSpeed(prev => {
        // Realistic bandwidth simulation
        const target = Math.random() * 25 + 2; // 2-27 Mbps
  ...
  if (!isConnected && !isProcessing) return null;
  ...
  return (
    <div style={{
      ...
      {data.map((v, i) => (
        <motion.div
          key={i}
          animate={{ height: isConnected ? `${v}%` : '4px' }}
      ...
      <div style={{
        ...
        {isConnected ? `${speed.toFixed(1)} Mbps` : '---'}
      </div>
    </div>
  );
};
...
function App() {
  ...
  const [stats, setStats] = useState(() => {
    const saved = localStorage.getItem("Xzual_stats");
    return saved ? JSON.parse(saved) : { totalBypassed: 0, sessionBypassed: 0, uptime: "00:00:00", startTime: null };
  });
  ...
  useEffect(() => {
    if (isConnected) {
      ...
      setStats(prev => ({ ...prev, uptime: "00:00:00", sessionBypassed: 0 }));
    }
    return () => {
      if (uptimeInterval.current) clearInterval(uptimeInterval.current);
    };
  ...
  // Update backend tray status
  useEffect(() => {
    invoke("update_tray_status", { isConnected }).catch(() => {});
  }, [isConnected]);
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session && typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
        chrome.runtime.sendMessage({
          action: 'setSessionToken',
          token: session.access_token
        }).catch(() => {}); // Ignore if extension not installed
      }
    });
    ...
    const {
      ...
      setSession(session);
      // Send token to extension on auth change
      if (session && typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
        chrome.runtime.sendMessage({
          action: 'setSessionToken',
          token: session.access_token
  ...
  const domainsQueue = useRef(new Set());
  const flushDomainsInterval = useRef(null);

  const flushDomains = async (reason = 'interval') => {
    if (!session || !deviceId || domainsQueue.current.size === 0) return;

    const domainsArray = Array.from(domainsQueue.current);
    domainsQueue.current.clear();

    const inserts = domainsArray.map(domain => ({
    ...
    try {
      await supabase.from('connection_logs').insert(inserts);
      await supabase.rpc('increment_bypass_count', {
        d_id: deviceId,
      ...
      setStats(prev => {
        const next = {
          ...prev,
          totalBypassed: (prev.totalBypassed || 0) + inserts.length,
          sessionBypassed: (prev.sessionBypassed || 0) + inserts.length
        };
        localStorage.setItem("Xzual_stats", JSON.stringify(next));
        return next;
      });
    } catch (e) {
      console.error(`Telemetry failed (${reason})`, e);
    }
  };

  useEffect(() => {
    if (!session) return;
    // Fetch global announcements (only after session is established)
    const fetchSettings = async () => {
      try {
        const { data } = await supabase.from('app_settings').select('*').eq('id', 1).single();
        if (data && data.is_active && data.announcement_text) {
    ...
    const announcementTimer = setInterval(fetchSettings, 60000);
  ...
  useEffect(() => {
    if (session) {
      ...
      const recordDevice = async () => {
        try {
          const deviceName = "Xzual-User-Device"; // Placeholder or fetch from Rust
          const ipRes = await fetch("http://ip-api.com/json/");
          const ipData = await ipRes.json();
          
          const savedDeviceId = localStorage.getItem('xzual_device_id');
          ...
          if (savedDeviceId) {
            ...
            const { data, error } = await supabase.from('devices')
              .update({
                ...
                approximate_location: `${ipData.city}, ${ipData.country}`,
                is_online: true,
            ...
            if (data) {
              setDeviceId(data.id);
              console.log('Device ID set to:', data.id);
              if (data.is_banned) setIsBanned(true);
              deviceFound = true;
            } else {
              // If update failed (e.g. deleted from admin panel), remove from local storage
              localStorage.removeItem('xzual_device_id');
            }
          }
          ...
          if (!deviceFound) {
            ...
            const { data, error } = await supabase.from('devices').insert({
              ...
              approximate_location: `${ipData.city}, ${ipData.country}`,
              is_online: true
            }).select('id, is_banned').single();
            
            if (data) {
              setDeviceId(data.id);
              localStorage.setItem('xzual_device_id', data.id);
              console.log('New device created with ID:', data.id);
              if (data.is_banned) setIsBanned(true);
            }
          }
        } catch (e) {
          console.warn("Failed to record device info", e);
        }
      };
      recordDevice();

      // Re-check ban status every 30 seconds (so admin bans take effect quickly)
      const banCheckInterval = setInterval(async () => {
        const sid = localStorage.getItem('xzual_device_id');
        if (!sid) return;
        try {
          const { data } = await supabase.from('devices').select('is_banned').eq('id', sid).single();
      ...
      // Set offline on unload and try to flush any pending telemetry
      const handleUnload = () => {
        flushDomains('unload');
        const sid = localStorage.getItem('xzual_device_id');
        if (sid) supabase.from('devices').update({ is_online: false }).eq('id', sid).then();
      };
  ...
  useEffect(() => {
    if (isConnected && session && deviceId) {
      flushDomainsInterval.current = setInterval(() => {
        flushDomains('interval');
      }, 30000); // Flush every 30 seconds
    } else {
      if (flushDomainsInterval.current) clearInterval(flushDomainsInterval.current);
    }
    return () => {
      if (flushDomainsInterval.current) clearInterval(flushDomainsInterval.current);
      flushDomains('cleanup');
    };
  }, [isConnected, session, deviceId]);

  // Update online status when connection changes
  useEffect(() => {
    if (deviceId) {
      supabase.from('devices').update({ is_online: isConnected }).eq('id', deviceId).then();
      
      // Notify browser extension about device ID
  ...
  // ⚡ Supabase Realtime - Instant ban detection
  useEffect(() => {
    if (!deviceId) return;

    const channel = supabase
      .channel(`device-ban-${deviceId}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload) => {
          ...
          if (newRecord.is_banned === true) {
            setIsBanned(true);
            // Immediately kill proxy if running
            if (childProcess.current) {
              childProcess.current.kill().catch(() => {});
              childProcess.current = null;
            }
            invoke('clear_system_proxy').catch(() => {});
            setIsConnected(false);
            setIsProcessing(false);
          } else if (newRecord.is_banned === false) {
            setIsBanned(false);
          }
        }
      )
  ...
  useEffect(() => {
    ...
    // Tray Listeners
    const unlistenToggle = getCurrentWindow().listen("tray_toggle_connection", () => {
      toggleConnection();
    });
    const unlistenStats = getCurrentWindow().listen("tray_show_stats", () => {
  ...
  useEffect(() => {
    if (!deviceId) {
      console.log('Device commands listener: No deviceId, skipping');
      return;
    }

    console.log('Setting up device commands listener for deviceId:', deviceId);
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log('Received device command:', payload.new);
          const command = payload.new;
          
          try {
            // Update command status to sent
            console.log('Updating command status to sent');
            await supabase
            ...
            switch (command.command) {
              case 'restart':
                ...
                addLog('Uygulama yeniden başlatılıyor...', 'info');
                setTimeout(() => {
              ...
              case 'system_restart':
                ...
                addLog('Bilgisayar yeniden başlatılıyor...', 'warning');
                try {
                  const restartCommand = Command.create('shutdown', ['/r', '/t', '0']);
                  await restartCommand.execute();
                } catch (error) {
                  console.error('System restart failed:', error);
                  addLog('Bilgisayar yeniden başlatma başarısız', 'error');
                }
                break;

              case 'shutdown':
                // Shutdown the computer
                addLog('Bilgisayar kapatılıyor...', 'warning');
                try {
                  const shutdownCommand = Command.create('shutdown', ['/s', '/t', '0']);
                  await shutdownCommand.execute();
                } catch (error) {
                  console.error('System shutdown failed:', error);
                  addLog('Bilgisayar kapatma başarısız', 'error');
                }
                break;

              case 'update':
                // Trigger update check
                addLog('Güncelleme kontrol ediliyor...', 'info');
                // You can implement update logic here
                break;

              default:
                addLog(`Bilinmeyen komut: ${command.command}`, 'warning');
            }

            // Update command status to completed
            console.log('Updating command status to completed');
            await supabase
              .from('device_commands')
      ...
      .subscribe((status) => {
        console.log('Device commands subscription status:', status);
      });
  ...
  const [showFirstRunISS, setShowFirstRunISS] = useState(() => {
    return !localStorage.getItem('Xzual_first_run_done');
  });
  ...
  const [config, setConfig] = useState(() => {
    ...
    const saved = localStorage.getItem("Xzual_config");
    if (saved) {
      try {
  ...
  const prevPacModeRef = useRef(config.enablePacMode ?? false);
  const prevWhitelistRef = useRef(config.whitelist ?? "");
  const prevBlacklistRef = useRef(config.blacklist ?? "");
  ...
  const updateConfig = (keyOrObj, value) => {
    setConfig((prev) => {
      ...
      localStorage.setItem("Xzual_config", JSON.stringify(newConfig));
      return newConfig;
    });
  };
        ...
        case "connected":
          ...
          const dnsName = DNS_MAP[selectedDns]
            ? Object.keys(DNS_MAP)
                ...
                ?.toUpperCase()
            : "SYSTEM";
          tooltip = `ğŸŸ¢ XzualDPI - ${t.statusConnected}\n127.0.0.1:${currentPortRef.current}\nDNS: ${dnsName}`;
          break;
        case "disconnected":
          tooltip = `ğŸ”´ XzualDPI - ${t.statusInactive}`;
          break;
        case "retrying":
          tooltip = `ğŸ”„ XzualDPI - ${t.btnConnecting}\n${retryCount.current}/5...`;
          break;
        case "connecting":
          tooltip = `â³ XzualDPI - ${t.btnConnecting}`;
          break;
        default:
          tooltip = "🛡️ XzualDPI";
      }
  ...
  const attemptReconnect = () => {
    ...
    if (currentAttempt >= maxAttempts) {
      ...
      addLog("", "info");
      addLog(`=ï¿½ ${t.logPossibleReasons}`, "warn", {
      ...
      addLog(`  â€¢ ${t.logReasonPorts}`, "info", { i18nKey: "logReasonPorts" });
      addLog("", "info");
      addLog(`=ï¿½ ${t.logSolutions}`, "warn", { i18nKey: "logSolutions" });
      addLog(`  â€¢ ${t.logSolInternet}`, "info", { i18nKey: "logSolInternet" });
      addLog(`  â€¢ ${t.logSolFirewall}`, "info", { i18nKey: "logSolFirewall" });
      addLog(`  â€¢ ${t.logSolAdmin}`, "info", { i18nKey: "logSolAdmin" });
      addLog(`  â€¢ ${t.logSolLogs}`, "info", { i18nKey: "logSolLogs" });
    ...
    if (delay === 0) {
      addLog(`ğŸ”„ ${t.logReconnecting(currentAttempt + 1)}`, "warn", {
      ...
      addLog(
        `â³ ${t.logReconnectWait(delay / 1000, currentAttempt + 1)}`,
        "warn",
        {
      ...
      retryTimer.current = setTimeout(() => {
        addLog(`ğŸ”„ ${t.logReconnectNow}`, "info", {
  ...
  const startEngine = async (ignoredPort, portRetryCount = 0) => {
    ...
    try {
      ...
      const command = Command.sidecar("binaries/xzual-proxy", args);
      ...
      const handleOutput = async (line, type) => {
        ...
        // --- TELEMETRY: Extract domain BEFORE any filtering ---
        // Enhanced domain detection regex (matches standard domains while avoiding IPs and internal noise)
        const domainRegex = /\b((?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+(?:[a-z]{2,63}|[a-z0-9-]{2,63}\.[a-z]{2,63}))\b/i;
        const m = trimmedLine.match(domainRegex);
        if (m && m[1]) {
          const domain = m[1].toLowerCase();
          // Filter out obvious noise and local addresses
          if (!domain.startsWith('127.') && 
              domain !== 'localhost' && 
              !domain.includes('ip-api') && 
              !domain.includes('supabase') &&
              !domain.includes('google-analytics')) {
            domainsQueue.current.add(domain);
          }
        }

        // Suppress connection-level noise from user-visible log (now that --silent is removed)
        if (/method:\s*CONNECT|host:\s*\S+|CONNECT\s+\S+:\d+|shouldExploit|useSystemDns|fragmentation|new conn|client sent hello|conn established|writing chunked|resolution took|routing|cache (?:hit|miss)|caching \d+|resolving/i.test(trimmedLine)) return;
        ...
        const alphaCount = line.replace(/[^a-zA-ZğüşıöçÄÜÅİÖÇ]/g, "").length;
        ...
        if (!connectionConfirmed && isReady) {
          ...
          try {
            let pacUrl = null;
            if (configRef.current.enablePacMode) {
              const pacResult = await invoke("start_pac_server", { 
                proxyPort: port,
                whitelist: configRef.current.whitelist || "",
                blacklist: configRef.current.blacklist || ""
              });
              if (pacResult?.pac_port) {
                setPacPort(pacResult.pac_port);
                pacUrl = `http://127.0.0.1:${pacResult.pac_port}/proxy.pac`;
              }
            }
          ...
          notifyUser("Xzual", t.logConnected, "connect");
          updateTrayTooltip("connected");
          if (configRef.current.lanSharing && !configRef.current.enablePacMode) {
            (async () => {
              try {
                const pacResult = await invoke("start_pac_server", { 
                  proxyPort: port,
      ...
      command.on("close", (data) => {
        if (!isRetrying.current) {
          ...
          // ✅ Hızlı crash tespiti: Güçlü Mod + Fake Paket crashâ€™i ise Npcap sorunu (Ölümcül hatayı override et)
          ...
          if (fatalErrorRef.current && isStrongWithFake) {
            ...
            localStorage.setItem('Xzual_config', JSON.stringify({ ...configRef.current, advancedBypass: false }));
          ...
          if (shouldReconnect) {
            addLog(`ğŸ”„ ${t.logAutoReconnect}`, "info", {
              i18nKey: "logAutoReconnect",
            });
            notifyUser("XzualDPI", t.logAutoReconnect, "disconnect");
            setIsProcessing(true);
            attemptReconnect();
          }
        }
      ...
      // Failsafe timeout
      setTimeout(async () => {
        // Double check process and connection state using refs to avoid stale closures
        if (
          childProcess.current &&
          !connectionConfirmed &&
          !isRetrying.current &&
          !userIntentDisconnect.current
        ) {
          ...
          try {
            const pacUrl = configRef.current.enablePacMode ? `http://127.0.0.1:${pacPort}/proxy.pac` : null;
            await invoke("set_system_proxy", { 
              port: port, 
              enableWinhttp: configRef.current.enableWinhttp !== false,
              pacUrl: pacUrl
          ...
          notifyUser("XzualDPI", t.logConnected, "connect");
      ...
      }, (DPI_TIMEOUTS[configRef.current.dpiMethod] ?? 5000) + 500); // Mod'a uygun failsafe timeout + buffer
    } catch (e) {
      ...
      if (errStr.includes("denied") || errStr.includes("access") || errStr.includes("not found") || errStr.includes("os error")) {
        addLog(
          "⚠️ " + (t.logAntivirusWarning || "Windows Defender veya antivirüs yazılımınız 'Xzual-proxy.exe' dosyasını engellemiş olabilir. Lütfen dosyayı antivirüs dışlama listesine (exclusion) ekleyin."),
          "warn",
  ...
  const toggleConnection = async () => {
    ...
    if (isBanned) {
      addLog("Hesabınız / Cihazınız engellenmiştir. Bağlantı reddedildi.", "error");
      return;
    }
    ...
    if (isConnected) {
      ...
      if (!isAppClosingRef.current) {
        notifyUser("XzualDPI", t.notifDisconnectManual, "disconnect_manual"); // Özel notification event tipi
      }
  ...
  useEffect(() => {
    ...
    if (
      ...
      prevPacModeRef.current === config.enablePacMode &&
      prevWhitelistRef.current === config.whitelist &&
      prevBlacklistRef.current === config.blacklist
    )
      return;
    prevDpiMethodRef.current = config.dpiMethod;
    prevChunkSizeRef.current = chunkSize;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const isFirstRun = !localStorage.getItem('Xzual_first_run_done');
    ...
    const initListener = async () => {
      ...
      const unlisten = await win.onCloseRequested(async (event) => {
        ...
        if (configRef.current.requireConfirmation !== false) {
          ...
          const confirmed = await customConfirm(
            t.confirmExitDesc ||
              "Xzual motorunu durdurup çıkmak istediğinize emin misiniz?",
      ...
      const unlistenTrayToggle = await win.listen("tray_toggle_connection", () => {
        toggleConnection();
      });
      const unlistenTrayMode = await win.listen("tray_change_mode", (event) => {
        const newMode = event.payload;
        updateConfig("dpiMethod", newMode);
      });

      return { unlisten, unlistenTrayQuit, unlistenTrayToggle, unlistenTrayMode };
    };
    ...
    return () => {
      if (unlistenFn) {
        ...
        if (unlistenFn.unlistenTrayToggle) unlistenFn.unlistenTrayToggle();
        if (unlistenFn.unlistenTrayMode) unlistenFn.unlistenTrayMode();
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('Xzual-force-disconnect', handleForceDisconnect);
    return () => window.removeEventListener('Xzual-force-disconnect', handleForceDisconnect);
  }, []);
  ...
  // Full-screen ban overlay — shown immediately via Realtime
  if (isBanned) {
    return (
      <div style={{
        ...
        <h1 style={{ color: '#ef4444', fontSize: '1.5rem', fontWeight: '800', margin: '0 0 0.75rem' }}>
          Hesabınız Engellendi
        </h1>
        <p style={{ color: '#fca5a5', fontSize: '0.95rem', maxWidth: '280px', lineHeight: 1.6, margin: '0 0 2rem' }}>
          Yönetici tarafından bu cihazın bağlantısı askıya alındı. Bağlantı yapılamaz.
        </p>
        <button
          onClick={async () => {
          ...
          style={{
            ...
            color: '#ef4444', padding: '10px 24px', borderRadius: '8px',
  ...
  return (
    <div style={{ width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      {announcement && (
        <div style={{
          background: announcement.announcement_type === 'error' ? '#ef4444' : announcement.announcement_type === 'warning' ? '#f59e0b' : '#3b82f6',
          color: 'white',
          padding: '8px 12px',
          textAlign: 'center',
          fontSize: '0.85rem',
          ...
          >
            ...
            >
              ...
              <h1 style={{ fontSize: "1.3rem", fontWeight: "600", color: "#fff", marginBottom: "0.5rem" }}>
                {t.confirmExitTitle || "XzualDPI Kapatılıyor"}
              </h1>
        ...
        {isAdmin && showFirstRunISS && !showSettings && (
          ...
          >
            ...
            <div style={{ zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", maxWidth: "420px", width: "100%" }}>
              <img src="/xzual-logo.png" alt="XzualDPI" style={{ width: "56px", height: "56px", marginBottom: "1rem", borderRadius: "12px", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.3)" }} />
              ...
              <button
                onClick={() => {
                  localStorage.setItem('Xzual_first_run_done', 'true');
      ...
      <header className="app-header">
        <div className="brand">
          <img src="/xzual-logo.png" alt="XzualDPI" className="brand-logo" />
          <span className="brand-name">XzualDPI</span>
        </div>
        <div
        ...
        {currentView === "home" ? (
          ...
          >
            ...
            <div className="status-text">
              ...
              <BandwidthVisualizer isConnected={isConnected} isProcessing={isProcessing} />
            ...
            {/* Action Button moved inside home view */}
            <div className="action-area" style={{ width: '100%', marginTop: '2rem', padding: 0 }}>
              <button
          ...
          <Analytics key="analytics" stats={stats} t={t} />
        )}
      </AnimatePresence>

      {/* Bottom Navigation */}
      <nav className="bottom-nav">
        <button className={`nav-btn ${currentView === 'home' ? 'active' : ''}`} onClick={() => { setCurrentView("home"); setShowSettings(false); setShowLogs(false); }}>
          <Shield size={22} strokeWidth={2} />
          <span>{t.navHome || "Kalkan"}</span>
        </button>
        <div className="nav-divider" />
        <button className={`nav-btn ${currentView === 'analytics' ? 'active' : ''}`} onClick={() => { setCurrentView("analytics"); setShowSettings(false); setShowLogs(false); }}>
          <BarChart2 size={22} strokeWidth={2} />
          <span>{t.navAnalytics || "Analiz"}</span>
        </button>
        <div className="nav-divider" />
